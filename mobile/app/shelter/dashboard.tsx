...
import { 
  ...
  ActivityIndicator, TextInput, Modal, Platform, Dimensions
} from 'react-native';
...
export default function ShelterDashboard() {
  ...
  const [confirm, setConfirm] = useState({
    visible: false, title: '', message: '', type: 'danger' as 'danger' | 'warning' | 'info',
    onConfirm: () => {}, icon: 'trash-outline' as any
  });
  ...
  const pickImage = async (isConvert = false) => {
    ...
    if (!result.canceled) {
      ...
      const file = { uri: asset.uri, name: asset.uri.split('/').pop(), type: 'image/jpeg' };
      if (!isConvert) setCatForm({ ...catForm, photo: file });
    }
  };
  ...
  const openEditModal = (cat: Cat) => {
    ...
    setEditId(cat.id);
    setCatForm({
      name: cat.name, breed: cat.breed, age: cat.age.toString(),
      gender: cat.gender, health_status: cat.health_status,
      description: cat.description, photo: null
    });
    setShowCatModal(true);
  };

  const submitCat = async () => {
    if (!catForm.name || !catForm.gender) return showPopup("error", "Peringatan", "Nama dan Gender wajib diisi!");
    if (!isEditing && !catForm.photo) return showPopup("error", "Peringatan", "Foto wajib diisi!");
    if (!shelterId) return;
    ...
    try {
      ...
      if (catForm.photo) formData.append('photo', catForm.photo as any);

      if (isEditing && editId) {
        await apiClient.put(`/cats/${editId}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
        showPopup("success", "Sukses", "Data kucing berhasil diperbarui");
      } else {
        await apiClient.post('/cats', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
        showPopup("success", "Sukses", "Kucing berhasil ditambahkan ke katalog");
      }
      setShowCatModal(false);
      fetchData();
    } catch (error) {
      showPopup("error", "Gagal", "Terjadi kesalahan saat menyimpan data.");
  ...
  const deleteCat = (id: number) => {
    setConfirm({
      visible: true,
      title: "Hapus Data",
      message: "Yakin ingin menghapus data kucing ini dari katalog?",
      type: 'danger',
      icon: 'trash-outline',
      onConfirm: async () => {
        try {
          ...
          showPopup("success", "Terhapus", "Data kucing berhasil dihapus.");
          fetchData();
        } catch (e) { showPopup("error", "Gagal", "Gagal menghapus data."); }
      }
    });
  };

  const returnToOwner = (lostCatId?: number) => {
    if (!lostCatId) return;
    setConfirm({
      visible: true,
      title: "Konfirmasi Pengembalian",
      message: "Apakah kucing ini sudah benar-benar diserahkan kembali ke pemiliknya?",
      type: 'info',
      ...
      onConfirm: async () => {
        try {
          ...
          setConfirm({ ...confirm, visible: false });
          showPopup("success", "Sukses", "Status kucing telah diperbarui menjadi dikembalikan.");
          fetchData();
        } catch (e) { showPopup("error", "Gagal", "Gagal memperbarui status."); }
      }
    });
  };
  ...
  const openConvertModal = (item: RescuedCat) => {
    ...
    setConvertForm({
      reportId: item.report_id || null, name: 'Kucing Rescue',
      gender: 'male', breed: 'Domestik', age: '12',
      description: item.description, photoPath: cleanPhotoPath
    });
  ...
  const submitConvert = async () => {
    ...
    try {
      ...
      showPopup("success", "Selesai", "Kucing telah dipindahkan ke katalog adopsi!");
      setShowConvertModal(false);
      fetchData();
      setActiveTab('cats');
    } catch (e) {
      showPopup("error", "Gagal", "Gagal memindahkan data rescue.");
    } finally { setIsSubmitting(false); }
  };
    ...
    <>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.tabContainer}>
            <TouchableOpacity style={[styles.tabBtn, activeTab === 'cats' && styles.tabBtnActive]} onPress={() => setActiveTab('cats')}>
                <Text style={[styles.tabText, activeTab === 'cats' && styles.tabTextActive]}>Siap Adopsi</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.tabBtn, activeTab === 'rescue' && styles.tabBtnActive]} onPress={() => setActiveTab('rescue')}>
                            ...
                            {cats.length === 0 ? (
                                ...
                                cats.map((cat) => (
                                    <View key={cat.id} style={styles.catCard}>
                                        <Image source={{ uri: resolveImageUrl(cat.photo || cat.image || null) || undefined }} style={styles.catImage} />
                                        <View style={styles.catInfo}>
                                            <View style={styles.catHeader}>
                                                <Text style={styles.catName}>{cat.name}</Text>
                                                <Ionicons name={cat.gender === 'male' ? 'male' : 'female'} size={16} color={cat.gender === 'male' ? '#3b82f6' : '#ec4899'} />
                                            </View>
            ...
            <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        <TextInput style={[styles.input, { height: 80 }]} multiline value={catForm.description} onChangeText={t => setCatForm({...catForm, description: t})} />
                        <TouchableOpacity style={[styles.submitBtn, isSubmitting && { opacity: 0.7 }]} onPress={submitCat} disabled={isSubmitting}>
...
const styles = StyleSheet.create({
  ...
  catCard: { backgroundColor: '#fff', borderRadius: 16, marginBottom: 15, flexDirection: 'row', overflow: 'hidden', elevation: 2, padding: 10, alignItems: 'center' },
